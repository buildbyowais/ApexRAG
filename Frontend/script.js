(function () {
    ...
    function updateDocBadge() {
        docIdBadge.textContent = documentId ? `ID: ${documentId.slice(0, 8)}` : 'No document';
    }
    ...
    async function performUpload(file) {
        ...
        try {
            ...
            fileMeta.textContent = `${size} MB · uploaded ${new Date().toLocaleTimeString()}`;

            uploadStatus.innerHTML = `<span class="text-success"><i class="bi bi-check-circle"></i> ${data.chunks || 0} chunks indexed</span>`;
            ...
            chatMessages.innerHTML = `<div class="empty-chat" id="emptyMessage"><i class="bi bi-chat-text"></i> Document ready! Ask a question.</div>`;
            questionInput.focus();

        } catch (err) {
            const msg = err.detail || err.message || 'Upload error';
            showToast(msg);
            uploadStatus.innerHTML = `<span class="text-danger">${msg}</span>`;
    ...
    dropzone.addEventListener('drop', (e) => {
        ...
        if (e.dataTransfer.files.length) {
            ...
            if (['.pdf', '.docx', '.txt'].some(ext => file.name.toLowerCase().endsWith(ext))) {
                ...
                showToast('Please upload PDF, DOCX, or TXT');
            }
        }
    });
    fileInput.addEventListener('change', (e) => {
        if (fileInput.files.length) performUpload(fileInput.files[0]);
        fileInput.value = '';
    });
    ...
    async function sendMessage() {
        ...
        if (!question || !documentId || isProcessing) return;
        ...
        thinkingEl.innerHTML = `<span>AI is thinking</span><span class="dot"></span><span class="dot"></span><span class="dot"></span>`;
        ...
        try {
            ...
            addMessage(data.answer, 'assistant', sources);
    ...
    function addMessage(text, type, sources = []) {
        ...
        if (type === 'assistant') {
            ...
            avatar.innerHTML = `<i class="bi bi-robot"></i> Assistant`;
            ...
            if (sources && sources.length) {
                ...
                sources.forEach((s) => {
                    ...
                    sourceItem.textContent =
                        `📄 ${s.file_name || 'doc'} (p.${s.page || '?'})`;

                    srcDiv.appendChild(sourceItem);
                });
    ...
    clearChatBtn.addEventListener('click', () => {
        chatMessages.innerHTML = `<div class="empty-chat" id="emptyMessage"><i class="bi bi-chat-text"></i> Chat cleared. Ask a new question.</div>`;
    });
